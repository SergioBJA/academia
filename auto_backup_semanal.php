<?php
/**
 * Script de copia de seguridad automática para la web completa - SEMANAL
 * Se recomienda ejecutar mediante Tarea Cron de Hostinger una vez por semana (ej. Domingos de madrugada).
 */
...
$backupFolder = __DIR__ . '/backupsemanal'; 
$timestamp = date('Y-m-d_H-i');
$backupFile = $backupFolder . "/backup_semanal_$timestamp.zip";

// 1. Verificar si existe la carpeta 'backupsemanal', si no, intertar crearla
if (!file_exists($backupFolder)) {
    if (!mkdir($backupFolder, 0755, true)) {
        die("Error: No se pudo crear/acceder a la carpeta de backups en $backupFolder.\nPor favor, crea manualmente la carpeta 'backupsemanal'.");
    }
}
...
// 6. Rotación de Backups: Mantener solo las últimas 4
$backups = glob($backupFolder . '/backup_semanal_*.zip');
if (count($backups) > 4) {
    // Ordenar por fecha de modificación (más antiguos primero)
    array_multisort(array_map('filemtime', $backups), SORT_ASC, $backups);
    $toDelete = count($backups) - 4;
...
// Mensaje de éxito
echo "✅ Backup SEMANAL completada con éxito.\n";

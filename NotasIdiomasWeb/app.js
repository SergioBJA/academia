...
const API_URL = '/api'; // Backend en Vercel (Python)
    ...
    return {
        ...
        async refreshData() {
            try {
                ...
                this.workflowChecks = data.workflow_checks || {};
                ...
                [this.trimesterChecks, this.workflowChecks].forEach(obj => {
        ...
        workflowChecks: {},
        async toggleWorkflowCheck(studentId, type) {
            if (!this.workflowChecks[studentId]) {
                this.workflowChecks[studentId] = { taller: false, cuota: false, intensivo: false };
            }
            
            const checks = { ...this.workflowChecks[studentId] };
            checks[type] = !checks[type];

            // Optimistic UI update
            this.workflowChecks[studentId] = checks;
            this.workflowChecks = { ...this.workflowChecks };
            this.searchContacts();

            try {
                await fetch(`${API_URL}?action=save_workflow_checks`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ action: 'save_workflow_checks', studentId, checks })
                });
            } catch (e) {
                console.error("Error saving workflow checks:", e);
            }
        },
            ...
            if (this.activeTab === 'cursos' && grado !== 'TODOS') {
                ...
                this.filteredContacts = filtered.map(s => {
                        ...
                        return {
                            ...
                            isAnulado: this.anulados.includes(id),
                            checks: this.trimesterChecks[id] || { t1: false, t2: false, t3: false, t3_date: null },
                            workflowChecks: this.workflowChecks[id] || { taller: false, cuota: false, intensivo: false },
                            note: this.teacherNotes[id] || '',
        ...
        async loadNotas(studentId, gradoParam) {
            ...
            try {
                ...
                const data = await res.json();
                this.selectedStudentNotas = (data.notas || [])
                    .map((r, index) => {
                        const row = this.mapNotaRow(r);
        ...
        isExamGrade(grado) {
            ...
            // Solo los cursos que tengan INTENSIVO puesto deben mostrar estos botones
            return g.includes('INTENSIVO');
        },

        getCurrentCourse() {
            if (this.selectedGrado && this.selectedGrado !== 'TODOS') return this.selectedGrado;
            if (this.selectedStudent && this.selectedStudent.course && this.selectedStudent.course !== 'TODOS') return this.selectedStudent.course;
            if (this.selectedStudentNotas && this.selectedStudentNotas.length > 0) return this.selectedStudentNotas[0].curso;
            return '';
        },

        calculateMediaSkill(skill) {
            const grado = (this.getCurrentCourse() || '').toUpperCase();
            ...
            const notas = [...this.selectedStudentNotas]
                ...
                .sort((a, b) => {
                    const da = a[`fecha_${skill}`] ? new Date(a[`fecha_${skill}`]).getTime() : 0;
                    const db = b[`fecha_${skill}`] ? new Date(b[`fecha_${skill}`]).getTime() : 0;
        ...
        calculateMediaGeneral() {
            const grado = this.getCurrentCourse();
        ...
        async downloadPDF(isBackground = false) {
            ...
            const actualGrado = this.getCurrentCourse() || '';
            const actualCourseName = (this.sheetMeta[actualGrado] && this.sheetMeta[actualGrado].c2) || actualGrado;
            const note = (this.sheetMeta[actualGrado] && this.sheetMeta[actualGrado].note) || '';
            const video = (this.sheetMeta[actualGrado] && this.sheetMeta[actualGrado].video) || '';
            const qr = (this.sheetMeta[actualGrado] && this.sheetMeta[actualGrado].qr) || '';
            ...
            const idMap = {
                ...
                'pdf-selected-grado': actualGrado,
                'pdf-course-name': actualCourseName
            };
            ...
            if (logoImg) {
                logoImg.src = this.sheetMeta[actualGrado]?.icon || '../CertificadosAgenciaTributaria/logo.png';
            }
            ...
            clone.querySelectorAll('[x-show*="isExamGrade"]').forEach(el => {
                const isExam = this.isExamGrade(actualGrado);
                const expr = el.getAttribute('x-show');
                if (expr && expr.includes('isExamGrade(getCurrentCourse())')) el.style.display = isExam ? 'block' : 'none';
                if (expr && expr.includes('!isExamGrade(getCurrentCourse())')) el.style.display = !isExam ? 'block' : 'none';
                if (expr === 'isExamGrade(selectedGrado)') el.style.display = isExam ? 'inline' : 'none';
                if (expr === '!isExamGrade(selectedGrado)') el.style.display = !isExam ? 'inline' : 'none';
            });

            // Parchear Status Aprobado (Color e Icono)
            const status = this.getAprobadoStatus();
            const statusBox = clone.querySelector('[x-text*="getAprobadoStatus().text"]'); // No existe este ID pero buscamos por atributo
            // Buscamos el contenedor del status por su clase característica
            const statusContainer = clone.querySelector('.w-12.h-12.bg-white.rounded-xl');
            if (statusContainer) {
                statusContainer.style.color = (status.text === 'APROBADO') ? '#10b981' : '#f43f5e';
                const templates = statusContainer.querySelectorAll('template');
                templates.forEach(t => {
                    const content = t.content.cloneNode(true);
                    const isAprobadoTemplate = t.getAttribute('x-if')?.includes('APROBADO');
                    if ((status.text === 'APROBADO' && isAprobadoTemplate) || (status.text !== 'APROBADO' && !isAprobadoTemplate)) {
            ...
            // 5. Contenedor temporal (INGENIERÍA FINAL)
            ...
            const opt = {
                margin: [0, 0, 0, 0], 
                filename: `Informe_${this.selectedStudent.nombre.replace(/\s+/g, '_')}.pdf`,
            ...
            return new Promise((resolve, reject) => {
                ...
                }, 1000); // 1 segundo de espera para seguridad total
            });
        },
